...
export function PageHero({
  ...
  const resolvedLocale = locale ?? (crumbs.some((crumb) => crumb.href?.startsWith('/en')) ? 'en' : 'th')
          ...
          <ol className="flex flex-wrap items-center gap-1 text-xs text-primary-foreground/70">
            {crumbs.map((c, i) => (
              <li key={i} className="flex items-center gap-1">
                {c.href ? (
                  <Link href={getLocalePath(c.href, resolvedLocale)} className="transition-colors hover:text-gold">
                    {c.label}
                  </Link>
                ) : (
